...
def generate_hash(message: bytearray) -> bytearray:
    ...
    for message_block in blocks:
        ...
        h6 = (h6 + g) % 2**32
        h7 = (h7 + h) % 2**32
        total = ((h0).to_bytes(4, 'big') + (h1).to_bytes(4, 'big') +
...
if __name__ == "__main__":
    generate_hash("abcdefghbcdefghicdefghijdefghijkefghijklfghijklmghijklmnhijklmnoijklmnopjklmnopqklmnopqrlmnopqrsmnopqrstnopqrstu")
    ...
    temp = [32219, 20249, 36894, 65108, 41185, 55030, 56187, 64524, 52585, 56508, 17718, 4619, 54502, 23304, 60528, 558]
    first = ""
    for i in range(int(len(temp)/2)):
        
        first = first + (temp[2*i] + temp[2*i+1] * ( 1<<16 )).to_bytes(4, "big").hex()
    print(first)

    test = [5799, 53083, 33664, 30895, 58782, 876, 37431, 31492, 39697, 2852, 31313, 59632, 17667, 44972, 59857, 31486]
    full = ""
    for i in range(int(len(test)/2)):
        full = full + (test[2*i] + test[2*i+1] * ( 1<<16 )).to_bytes(4, "big").hex()
    print(full)